...
LOGGING_CFG = {
    ...
    'handlers': {
        'file': {
            ...
            'filename': os.path.join(tempfile.gettempdir(), 'skeleton.log')
        },
        'console': {
            'level': 'CRITICAL',
...
def tempfile_name():
    ...
    ret = os.path.join(tempfile.gettempdir(), 'skeleton.log')
    if os.access(ret, os.F_OK) and not os.access(ret, os.W_OK):
        ...
        ret = tempfile.mkstemp(prefix='lmit', suffix='.tmp', text=True)
